...
import br.unipampa.lesa.Triangulo;


public class TrianguloPart {

	@Inject
	private MDirtyable dirty;
	private Text valorLargura;
	private Text txtResultado;
	private Text valorComprimento;
	...
	public void createComposite(Composite parent) {
		...
		lblNewLabel.setImage(ResourceManager.getPluginImage("PrimeiroProjetoEmRCP", "icons/trianguloIcone.png"));

		Label lblNewLabel_1 = new Label(parent, SWT.NONE);
		lblNewLabel_1.setBounds(10, 164, 116, 23);
		lblNewLabel_1.setText("Largura (cm):");

		valorLargura = new Text(parent, SWT.BORDER);
		valorLargura.setBounds(129, 161, 76, 21);

		Button btnCalcularrea = new Button(parent, SWT.NONE);
		btnCalcularrea.setFont(SWTResourceManager.getFont("Segoe UI", 9, SWT.BOLD));
		btnCalcularrea.setBounds(10, 193, 135, 25);
		...
		txtResultado.setBounds(175, 193, 145, 23);

		Label label = new Label(parent, SWT.NONE);
		label.setFont(SWTResourceManager.getFont("Segoe UI", 12, SWT.BOLD));
		label.setBounds(151, 193, 18, 20);
		label.setText("=");
		
		Label lblValorDaLargura = new Label(parent, SWT.NONE);
		lblValorDaLargura.setText("Comprimento (cm):");
		lblValorDaLargura.setBounds(10, 135, 116, 23);
		
		valorComprimento = new Text(parent, SWT.BORDER);
		valorComprimento.setBounds(129, 132, 76, 21);

		btnCalcularrea.addListener(SWT.MouseUp, new Listener() {
			@Override
			public void handleEvent(Event event) {
				try {
					double comprimento = Double.parseDouble(valorComprimento.getText());
					double largura = Double.parseDouble(valorLargura.getText());

					Triangulo triangulo = new Triangulo(comprimento, largura);
					txtResultado.setText(String.valueOf(triangulo.recuperarArea()));
